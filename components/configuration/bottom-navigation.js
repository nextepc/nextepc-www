...
export default class BottomNavigation extends Component {
  render () {
    const { configuration, post } = this.props
    const currIdx = configuration.findIndex(config => ( config.data.title == post.data.title ))
    const prev = configuration[currIdx - 1]
    const next = configuration[currIdx + 1]
